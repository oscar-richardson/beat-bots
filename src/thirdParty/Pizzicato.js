(function(root) {
    ...
    if (commonJS) module.exports = Pizzicato;
    else if (amd) define([], Pizzicato);
    else root.Pizzicato = root.Pz = Pizzicato;
    ...
    Pizzicato.Util = {
        ...
        isInRange: function(arg, min, max) {
            if (!Pz.Util.isNumber(arg) ||
                !Pz.Util.isNumber(min) ||
                !Pz.Util.isNumber(max)
            )
                return false;
        ...
        isAudioBufferSourceNode: function(audioNode) {
            return (
                audioNode && audioNode.toString() === "[object AudioBufferSourceNode]"
            );
        },
        ...
        normalize: function(num, floor, ceil) {
            if (!Pz.Util.isNumber(num) ||
                !Pz.Util.isNumber(floor) ||
                !Pz.Util.isNumber(ceil)
            )
                return;
    ...
    Pizzicato.Events = {
         ...
         */
        off: function(name) {
            if (name) this._events[name] = undefined;
            else this._events = {};
        },
    };
    Pizzicato.Sound = function(description, callback) {
        ...
        var hasOptions =
            util.isObject(description) && util.isObject(description.options);
        ...
        if (descriptionError) {
            ...
            throw new Error(
                "Error initializing Pizzicato Sound: " + descriptionError
            );
        }
        ...
        this.attack =
            hasOptions && util.isNumber(description.options.attack) ?
            description.options.attack :
            defaultAttack;
        this.volume =
            hasOptions && util.isNumber(description.options.volume) ?
        ...
        if (!description) initializeWithWave.bind(this)({}, callback);
        ...
        function getDescriptionError(description) {
            ...
            if (util.isObject(description)) {
                if (!util.isString(description.source) ||
                    supportedSources.indexOf(description.source) === -1
                )
                    return "Specified source not supported. Sources can be wave, file, input or script";

                if (
                    description.source === "file" &&
                    (!description.options || !description.options.path)
                )
                    return "A path is needed for sounds with a file source";

                if (
                    description.source === "script" &&
                    (!description.options || !description.options.audioFunction)
                )
        ...
        function initializeWithWave(waveOptions, callback) {
            ...
            this.getRawSourceNode = function() {
                var frequency = this.sourceNode ?
                    this.sourceNode.frequency.value :
                    waveOptions.frequency;
        ...
        function initializeWithUrl(paths, callback) {
            ...
            request.onreadystatechange = function(event) {
                if (request.readyState === 4 && request.status !== 200) {
                    console.error(
                        "Error while fetching " + paths[0] + ". " + request.statusText
                    );
                }
            };
            request.send();
        ...
        function initializeWithInput(options, callback) {
            ...
            if (!!navigator.mediaDevices.getUserMedia)
                ...
                .then(handleStream)
                .catch(handleError);
            else navigator.getUserMedia({ audio: true }, handleStream, handleError);
        }

        function initializeWithFunction(options, callback) {
            var audioFunction = util.isFunction(options) ?
                options :
                options.audioFunction;
            var bufferSize =
            ...
            if (!bufferSize) {
                try {
                    // Webkit does not automatically chose the buffer size
        ...
        function initializeWithSoundObject(options, callback) {
            ...
            if (
                options.sound.sourceNode &&
                Pz.Util.isOscillator(options.sound.sourceNode)
            ) {
                this.sourceNode = this.getRawSourceNode();
    ...
    Pizzicato.Sound.prototype = Object.create(Pizzicato.Events, {
        ...
        pause: {
            ...
            value: function() {
                ...
                if (this.sourceNode.buffer)
                    this.offsetTime =
                    elapsedTime %
                    (this.sourceNode.buffer.length / Pz.context.sampleRate);
                else this.offsetTime = elapsedTime;
        ...
        removeEffect: {
            ...
            value: function(effect) {
                ...
                var previousNode =
                    index === 0 ? this.fadeNode : this.effectConnectors[index - 1];
                previousNode.disconnect();
                ...
                else targetNode = this.effects[index];
        ...
        connectEffects: {
            ...
            value: function() {
                ...
                for (var i = 0; i < this.effects.length; i++) {
                    ...
                    var destinationNode = isLastEffect ?
                        this.masterVolume :
        ...
        getSourceNode: {
            ...
            value: function() {
                if (!!this.sourceNode) {
                    ...
                    previousSourceNode.gainSuccessor.gain.setValueAtTime(
                        previousSourceNode.gainSuccessor.gain.value,
                        Pz.context.currentTime
                    );
                    previousSourceNode.gainSuccessor.gain.linearRampToValueAtTime(
                        0.0001,
                        Pz.context.currentTime + 0.2
                    );
                    setTimeout(function() {
        ...
        stopWithRelease: {
            ...
            value: function(callback) {
                ...
                var stopSound = function() {
                    return Pz.Util.isFunction(node.stop) ?
                        node.stop(0) :
                        node.disconnect();
                };
    ...
    Pizzicato.Group.prototype = Object.create(Pz.Events, {
        ...
        removeEffect: {
            ...
            value: function(effect) {
                ...
                var previousNode =
                    index === 0 ? this.mergeGainNode : this.effectConnectors[index - 1];
                previousNode.disconnect();
    ...
    Pizzicato.Effects.Compressor = function(options) {
        ...
        this.inputNode = this.compressorNode =
            Pizzicato.context.createDynamicsCompressor();
        this.outputNode = Pizzicato.context.createGain();
    ...
    Pizzicato.Effects.Distortion.prototype = Object.create(baseEffect, {
        ...
        adjustGain: {
            ...
            enumerable: false,
            value: function() {
                var gain = Pz.Util.isNumber(this.options.gain) ?
                    parseInt(this.options.gain * 100, 10) :
                    50;
                var n_samples = 44100;
                ...
                for (var i = 0; i < n_samples; ++i) {
                    x = (i * 2) / n_samples - 1;
                    curve[i] =
                        ((3 + gain) * x * 20 * deg) / (Math.PI + gain * Math.abs(x));
                }
    ...
    Pizzicato.Effects.Convolver = function(options, callback) {
        ...
        request.onreadystatechange = function(event) {
            if (request.readyState === 4 && request.status !== 200) {
                console.error(
                    "Error while fetching " + options.impulse + ". " + request.statusText
                );
            }
        };
    ...
    function buildImpulse() {
        ...
        var impulse = Pizzicato.context.createBuffer(
            2,
            length,
            Pz.context.sampleRate
        );
        ...
        for (i = 0; i < length; i++) {
            ...
            impulseL[i] =
                (Math.random() * 2 - 1) * Math.pow(1 - n / length, this.decay);
            impulseR[i] =
                (Math.random() * 2 - 1) * Math.pow(1 - n / length, this.decay);
        }
    ...
    DiodeNode.prototype.setCurve = function() {
        ...
        for (
            i = _i = 0, _ref = wsCurve.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i
        ) {
            ...
            if (v <= this.vb) {
                ...
                value =
                    this.h * (Math.pow(v - this.vb, 2) / (2 * this.vl - 2 * this.vb));
            } else {
                value =
                    this.h * v -
                    this.h * this.vl +
                    this.h *
                    (Math.pow(this.vl - this.vb, 2) / (2 * this.vl - 2 * this.vb));
            }
            wsCurve[i] = value;
        }
    ...
    Pizzicato.Effects.Quadrafuzz.prototype = Object.create(baseEffect, {
        lowGain: {
            ...
            set: function(lowGain) {
                ...
                this.overdrives[0].curve = getDistortionCurve(
                    Pz.Util.normalize(this.lowGain, 0, 150)
                );
            },
        },
        ...
        midLowGain: {
            ...
            set: function(midLowGain) {
                ...
                this.overdrives[1].curve = getDistortionCurve(
                    Pz.Util.normalize(this.midLowGain, 0, 150)
                );
            },
        },
        ...
        midHighGain: {
            ...
            set: function(midHighGain) {
                ...
                this.overdrives[2].curve = getDistortionCurve(
                    Pz.Util.normalize(this.midHighGain, 0, 150)
                );
            },
        },
        ...
        highGain: {
            ...
            set: function(highGain) {
                ...
                this.overdrives[3].curve = getDistortionCurve(
                    Pz.Util.normalize(this.highGain, 0, 150)
                );
            },
        },
    });
